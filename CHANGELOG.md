...
### 비호텔 탭 검색 동작 (2026-07-20)

- 항공/액티비티/렌터카 탭을 정적 UI에서 **동작하는 검색 패널**로 전환
- 항공: 출발/도착 공항 주요팝오버+자동완성, 출/도착지 변경, 왕복/편도/다구간
  (왕복=범위·편도=단일 달력), 인원 팝오버(좌석등급 4종 + 성인/아동/유아)
- 렌터카: 인수/반납 도시 자동완성, 인수→반납 단일 달력 연속 선택, 운전자
  생년월일, 차량 보험 select, 반납장소 같음/다름
- 액티비티: 여행지/상품명 입력 + 카테고리 칩 선택
- CalendarPopover에 single/range 모드·라벨·위치 오버라이드 추가(재사용)
- 공항 Mock(원본 공항 팝오버 실측), 검색 URL 직렬화 3종, 결과 자리 페이지
  (/flight·/activity·/rentalcar/search-result — 조건 수신 표시)
- 단위 테스트 6건 추가(직렬화 24건 전체 통과)
